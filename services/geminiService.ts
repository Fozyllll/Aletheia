...
export const generateQuotes = async (count: number = 3, lang: Language = 'English', exclude: string[] = []): Promise<Quote[]> => {
  ...
  try {
    const response = await ai.models.generateContent({
      ...
      1. Provide a balanced mix of real historical quotes (from famous philosophers, writers, or thinkers) and original AI quotes.
      2. For real quotes, use the actual author's name and set "isAI": false.
      3. For AI quotes, use "Aletheia" as the author and set "isAI": true.
      4. Ensure the quotes are profound and fit the aesthetic of a high-end wisdom app.
...
export const generateQuoteImage = async (prompt: string): Promise<string> => {
  ...
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey || apiKey === "undefined" || apiKey === "") return `https://picsum.photos/seed/${Math.random()}/1080/1920`;
